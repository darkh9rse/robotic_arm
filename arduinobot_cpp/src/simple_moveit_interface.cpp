...
void move_robot(const std::shared_ptr<rclcpp::Node> node)
{
    ...
    std::vector<double> arm_joint_goal {1.57, 0.0, 0.0, 0.0};
    std::vector<double> gripper_joint_goal {-0.7, 0.7};

    bool arm_within_bounds = arm_move_group.setJointValueTarget(arm_joint_goal);
    bool gripper_within_bounds = arm_move_group.setJointValueTarget(gripper_joint_goal);

    if(!arm_within_bounds || !gripper_within_bounds)
    {
        RCLCPP_WARN(rclcpp::get_logger("rclcpp"),
                    "Target joint positon(s) were outside of limits, but we will plan and clamp to the limits" );
        return;
    }
